...
Public Class ConfigFormGPT
    ' 用于存储和访问设置的属性
    Public Property GPTApiUrl As String
    Public Property GPTToken As String
        ...
        Try
            Using key As RegistryKey = Registry.CurrentUser.OpenSubKey(REG_PATH)
                If key IsNot Nothing Then
                    Dim savedUrl = key.GetValue("GPTApiUrl")
                    Dim savedToken = key.GetValue("GPTToken")
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        ' 保存值到属性
        GPTApiUrl = txtApiUrl.Text.Trim()
        GPTToken = txtToken.Text.Trim()

        ' 保存到注册表
        Try
            Using key As RegistryKey = Registry.CurrentUser.CreateSubKey(REG_PATH)
                key.SetValue("GPTApiUrl", GPTApiUrl)
                key.SetValue("GPTToken", GPTToken)
            End Using
